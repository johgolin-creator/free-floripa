        ...
        <div className="flex min-w-0 items-center gap-2 rounded-lg border border-slate-200 bg-brand-charcoal p-1.5 shadow-sm">
          <div className="flex min-w-0 items-center gap-2 rounded-md bg-navy-950 px-3 py-2 text-white">
            ...
            <div className="min-w-0">
              <strong className="block text-xs">{state.activeRole === "empresa" ? "Empresa" : "Trabalhador"}</strong>
              {email && <span className="block max-w-[150px] truncate text-[0.68rem] text-slate-300">{email}</span>}
            </div>
          </div>
          <button
            type="button"
    ...
    return (
      <div className="grid grid-cols-2 rounded-lg border border-slate-200 bg-brand-charcoal p-1 shadow-sm">
        <button
          ...
          className={`inline-flex min-h-9 items-center justify-center gap-1.5 rounded-md px-2 text-xs font-black ${
