export type StatusBadgeType = "application" | "shift" | "schedule" | "job";
...
function toneClass(type: StatusBadgeType, status: string) {
  ...
  if (type === "shift") {
    if (status === "Fez check-in") return "badge bg-navy-950 text-white";
    if (status === "Finalizou o turno") return "badge bg-aqua-100 text-aqua-700";
    return "badge";
  }
...
const legendCopy: Record<StatusBadgeType, { status: string; meaning: string }[]> = {
  ...
  shift: [
    { status: "Ainda não chegou", meaning: "o turno ainda não começou" },
    { status: "Fez check-in", meaning: "o profissional confirmou presença no local" },
    { status: "Finalizou o turno", meaning: "o turno foi encerrado" }
  ],
  schedule: [
